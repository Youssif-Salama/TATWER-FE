...
import { setRefreshOnDeleteEstate } from "@/store/slices/GlobalSlice"
import { AppDispatch } from "@/store/store"
import { useState } from "react"
import { useDispatch } from "react-redux"
import { IoClose } from "react-icons/io5";
import { DeleteMultipleEstatesApi } from "@/api/estate/DeleteEstateApi";

const DeleteEstate = ({row,setCatchSelectedRows}:{row:any,setCatchSelectedRows:any}) => {
  const [loading, setLoading] = useState(false);
  const dispatch:AppDispatch=useDispatch();
  const deleteCurrentContractSystem=async()=>{
   let result=await DeleteMultipleEstatesApi(row,setLoading);
   result && dispatch(setRefreshOnDeleteEstate(Math.random()));
