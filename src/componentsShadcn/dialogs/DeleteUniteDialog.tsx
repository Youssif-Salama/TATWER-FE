...
import {  setRefreshOnDeleteUnite } from "@/store/slices/GlobalSlice"
import { AppDispatch } from "@/store/store"
import { useState } from "react"
import { useDispatch } from "react-redux"
import { IoClose } from "react-icons/io5";
import { DeleteMultipleUnitesApi } from "@/api/unites/DeleteMultipleUnitesApi";
import { successToaster } from "@/utils/ReactToatify";

const DeleteUniteDialog = ({row,setCatchSelectedRows}:{row:any,setCatchSelectedRows:any}) => {
  const [loading, setLoading] = useState(false);
  const dispatch:AppDispatch=useDispatch();
  const deleteMultipleUnites=async()=>{
   let result=await DeleteMultipleUnitesApi(row,setLoading);
   result && dispatch(setRefreshOnDeleteUnite(Math.random()));
...
export default DeleteUniteDialog
