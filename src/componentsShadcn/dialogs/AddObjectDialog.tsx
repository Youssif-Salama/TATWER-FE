...
import { setResfreshObjects } from "@/store/slices/GlobalSlice";
import { AddObjectApi } from "@/api/objects/AddObjectApi";
...
const AddObjectDialog = () => {
  ...
  const formik = useFormik<TaxFormProps>({
    ...
    onSubmit: async (values: TaxFormProps, { resetForm }) => {
      const result = await AddObjectApi(values, setLoading);
      if (result) {
        dispatch(setResfreshObjects(Math.random()));
        resetForm();
      }
    },
  });
