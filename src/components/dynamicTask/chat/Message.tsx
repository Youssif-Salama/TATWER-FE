...
const Message = ({ message, key,setCurrentImageToDisplay }: { message: any; key: any ,setCurrentImageToDisplay:any}) => {
  ...
  return (
    <div className={`w-full flex flex-col mb-4 ${(message?.EmpId?._id == currentEmpData?._id ) ? "items-start" : " items-end"}
    `} key={key} ref={lastMessage}>
      <div className={`flex gap-1 relative  max-md:w-[60%] w-[30%] ${(message?.EmpId?._id == currentEmpData?._id) ? "flex-row" : "flex-row-reverse"} `}>
        ...
        >
          {message?.Message}
          {message?.FileId &&<div className={`grid w-full gap-2 ${message?.FileId?.File?.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}>
            ...
            <div key={file._id} className="relative">
              ...
              <div className={`bg-white p-1 absolute  rounded-full cursor-pointer
              ${(message?.EmpId?._id == currentEmpData?._id) ? "top-2 left-2" : "top-2 right-2"}
              `}
              onClick={()=>{
                if(file.path.includes("pdf") || file.path.includes("jpg") || file.path.includes("png") || file.path.includes("jpeg")|| file.path.includes("webp")|| file.path.includes("svg")){
                  setCurrentImageToDisplay(file.path)
                }
              }}
              >
              <FaEye className={`
                 ${(message?.EmpId?._id == currentEmpData?._id) ? " text-[#0077bc]" : ""}
                `}/>
              </div>
            </div>
          ))}</div>}
        ...
        <div className={`p-1 cursor-pointer absolute top-0 -left-8 h-[150px] ${(message?.EmpId?._id == currentEmpData?._id) && currentEmpData?.Role!=="super_admin" ? "" : "hidden"}`}>
          <BsThreeDotsVertical className="text-[#0077bc]"
          onClick={()=>{
            setOpenMessageEdit(!openMessageEdit)
          }}
          />
          <div className={`text-[12px]
            bg-[#0077bc] absolute top-[15%] right-0 p-1 px-3 mt-1 rounded-xl text-white ${openMessageEdit && "hidden"}`}>
            <p
            onClick={async() => {
            if( message?.EmpId?._id == currentEmpData?._id && currentEmpData?.Role=="employee")
            {
      ...
      <div className={`text-[#fff] py-1 flex  items-center gap-2
        ${(message?.EmpId?._id == currentEmpData?._id) ? "flex-row" : " flex-row-reverse"}
        `}>
        <div className={`border rounded-full px-3 py-1
         ${(message?.EmpId?._id == currentEmpData?._id) ? " bg-[#0077bc]" : " bg-slate-200 text-[#1f1f1f]"}
         `}
        >
