...
const ContractCurrentFiles = ({
  ...
  useEffect(() => {
    if (ContractId) {
      getContractCurrentFiles(ContractId);
    }
  }, [ContractId, refreshContractFiles]);

  useEffect(() => {
    setFixedFiles([...files]);
    setCurrentNames([...names]);
    setNewFiles([...allFiles]);
  }, [files, names, refreshContractFiles, allFiles]);
  ...
  const updateContractFiles = async (data: any, id: any) => {
    ...
    result && dispatch(setRefreshContractFiles(Math.random()));
  };
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={() => {
                      // window.open(`http://92.113.39.182:10000${file.path}`, "_blank");
                      setSelectedFileToShow(
                        `http://92.113.39.182:10000${file.path}`
                      );
                    }}
                  >
                    <FaEye />
...
export default ContractCurrentFiles;
