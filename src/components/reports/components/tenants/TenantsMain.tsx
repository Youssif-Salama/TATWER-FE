...
const TenantsMain = () => {
  ...
  const getAllTenants=async()=>{
    const result=await GetAllSystemsForReports("tenant",tenant,setLoading,page,startDate,endDate);
  ...
  useEffect(()=>{
    let temp={
        ...
        "العنوان": allSystems[0]?.ContractId?.AddressId?.Town || "-",
        ...
        "العقار مدينه": allSystems[0]?.EstateAddress?.Town || "-",
  ...
  const getAllSystemsForPrint = async () => {
    const result = await GetAllSystemsForReports("tenant", tenant, setAllLoading, allPage, startDate, endDate);
...

{
  ...
  <div>
    <button
    onClick={()=>{
      generatePdfFromArray(dataToExport,headers,keys,"A1","تطوير-تقرير-المؤجر",extraContent)
    }}
  ...
  <div>
    <button
    onClick={()=>{
      generateArabicExcelFromArray(dataToExport,headers,keys,"تطوير-تقرير-المؤجر",extraContent)
    }}
    className="bg-green-500 text-[14px] p-1 px-4 hover:opacity-75 duration-200 transition-all text-white">excel</button>
      ...
      <div className="my-6">
        ...
        {
           (allSystems && allSystems.length>0 && allSystems[0]?.ContractId) &&
        <TenantsTable allSystems={allSystems}/>
        }
      </div>
        {
