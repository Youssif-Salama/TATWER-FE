import { GetAllEstatesForSearch } from "@/api/estate/GetAllEstatesForSearch";
...
const EstateSearch = ({setTenantId}: {setTenantId: any}) => {
  ...
  const getAllTenants = async () => {
    setLoading(true);
    const result = await GetAllEstatesForSearch(
      page,
      setLoading,
      searchKeyWord,
      searchValue,
        ...
        <div className="flex items-center max-w-[339px]">
          <div className="bg-gray-200 flex items-center justify-center px-1 py-2 relative">
            ...
            >
              ...
              <option className="hover:bg-[#0077bc] p-1 cursor-pointer" value="EstateName">الاسم</option>
              <option className="hover:bg-[#0077bc] p-1 cursor-pointer" value="TitleDeedNumber">رقم العقد</option>
            </select>
          </div>
          <input
            ...
            {contracts.length > 0 ? (
              contracts.map((item, index) => (
                <p
                  ...
                  onClick={() => {handleClick(item.EstateName)
                    setTenantId(item)
                  }}
                >
                  {item.EstateName}
