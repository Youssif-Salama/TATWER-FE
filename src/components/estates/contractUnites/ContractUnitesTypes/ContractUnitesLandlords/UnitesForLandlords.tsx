...
const UnitesForLandlords = ({setLandlordUnit}: {setLandlordUnit: any}) => {
  ...
  const getAllTenants = async () => {
    setLoading(true);
    const result = await GetSpecificEstateUnitesApi(
      estateId,
      setLoading,
      page,
      searchKeyWord,
        ...
        <div className="flex items-center max-w-[339px]">
          <div className="bg-gray-200 flex items-center justify-center px-1 py-2 relative">
            ...
            <option className="hover:bg-[#0077bc] p-1 cursor-pointer">اختر كلمه البحث</option>
              <option className="hover:bg-[#0077bc] p-1 cursor-pointer" value="UniteName">الاسم</option>
              <option className="hover:bg-[#0077bc] p-1 cursor-pointer" value="UnitSpace">المساحة</option>
            </select>
          </div>
          <input
            ...
            {unites.length > 0 ? (
              unites.map((item, index) => (
                <p
                  ...
                  onClick={() => {handleClick(item.UniteName)
                    setLandlordUnit(item._id);
                  }}
                >
                  {item.UniteName}
                </p>
...
export default UnitesForLandlords;
